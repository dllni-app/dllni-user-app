...
class _SmOrderDetailsScreenState extends State<SmOrderDetailsScreen> {
  ...
  Future<void> _fetchTracking() async {
    ...
    final Either<Failure, FetchRestaurantOrderTrackingModel> response = await getIt<FetchStoreOrderTrackingUseCase>()(
  ...
  int _statusStep() {
    final status = (_tracking?.latestToStatus ?? widget.args.order.status ?? '').toLowerCase();
    if (status.contains('delivered') || status.contains('completed')) return 4;
    if (status.contains('out_for_delivery') || status.contains('dispatch')) return 3;
  ...
  Widget build(BuildContext context) {
    ...
    final hasDelivery = (order.fulfillment?.type ?? '').toLowerCase() == 'delivery';
            ...
            : RefreshIndicator(
                ...
                child: ListView(
                  ...
                  children: [
                      ...
                      Padding(
                        ...
                        child: Material(
                          ...
                          child: Padding(
                            ...
                            child: Row(
                              children: [
                                Expanded(child: Text(_loadError!, style: const TextStyle(color: Color(0xff991B1B), fontSize: 13))),
                                TextButton(onPressed: _fetchTracking, child: const Text('إعادة المحاولة')),
                              ],
                            ),
                          ),
                        ),
                    ...
                    OrderTrackingStatusCard(hasDelivery: hasDelivery, step: _statusStep()),
                    const SizedBox(height: 12),
