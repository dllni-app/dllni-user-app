...
import '../widgets/order_voting_success_dialog.dart';
import '../widgets/personal_details_app_bar.dart';

enum _VotingMode { create, existingPolls }

@AutoRoutePage()
class OrderVotingScreen extends StatefulWidget {
  const OrderVotingScreen({super.key});

  @override
  State<OrderVotingScreen> createState() => _OrderVotingScreenState();
}

class _OrderVotingScreenState extends State<OrderVotingScreen> {
  ...
  void _showCreateSuccessDialog(int voteId) {
    showDialog<void>(
      context: context,
      builder: (_) {
        return OrderVotingSuccessDialog(
          onContinueTap: () {
