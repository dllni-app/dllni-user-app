...
class OfferCard extends StatelessWidget {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Stack(
          ...
          children: [
            Padding(
              padding: EdgeInsetsDirectional.only(start: 12, end: 12, bottom: 12, top: 20),
              child: Row(
                children: [
                      ...
                      children: [
                        Row(
                          children: [
                            Expanded(
                              child: AppText(
                                (data.restaurantName ?? '').trim().isEmpty ? 'مطعم' : data.restaurantName!,
                                textAlign: TextAlign.start,
                                style: TextStyle(color: Color(0xFF1A1A1A), fontSize: 12, fontWeight: FontWeight.w700),
                              ),
                            ),
            ...
            Positioned(
              ...
              child: Visibility(
                ...
                child: Container(
                  ...
                  child: AppText(
                    data.urgencyTag ?? '',
                    style: TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w700, height: 15 / 10),
                  ),
                ),
              ),
            ),
  ...
  void _onTap(BuildContext context) {
    ...
    if (restaurant != null && (restaurant.id ?? 0) > 0) {
      final preview = RestaurantPreviewData.fromHomeExclusiveOfferRestaurant(restaurant);
      context.pushRoute(
        '/rs_store',
        arguments: StoreDetailsScreenParams(restaurantId: restaurant.id!, preview: preview),
      );
      return;
    }
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) =>
            RsHomeExclusiveOfferProductsScreen(title: _description.isNotEmpty ? _description : 'العرض', products: data.products ?? const []),
      ),
    );
  }
