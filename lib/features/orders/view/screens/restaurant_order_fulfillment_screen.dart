      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: BlocConsumer<OrdersBloc, OrdersState>(
            listenWhen: (previous, current) => previous.placeOrderStatus != current.placeOrderStatus,
            listener: (context, state) {
              if (state.placeOrderStatus == BlocStatus.success) {
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم تاكيد الطلب بنجاح')));
                context.pop(true);
              } else if (state.placeOrderStatus == BlocStatus.failed) {
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(state.placeOrderErrorMessage ?? 'تعذر تاكيد الطلب حالياً.')));
              }
            },
            builder: (context, state) {
              final isDelivery = (state.selectedFulfillmentType ?? 'delivery') == 'delivery';
              final isPlacingOrder = state.placeOrderStatus == BlocStatus.loading;
              final amounts = state.restaurantCart?.amounts;
              final subtotal = state.couponData?.amounts?.subtotal ?? amounts?.subtotal ?? 0;
              final discount = state.couponData?.amounts?.discount ?? (subtotal - (amounts?.total ?? subtotal));
              final deliveryFee = isDelivery ? 0.0 : 0.0;
              final total = state.couponData?.amounts?.total ?? amounts?.total ?? 0;
              return Column(
                children: [
                      ...
                      child: Column(
                        children: [
                          ...
                          if (isDelivery)
                            ...
                            _LocationCard(
                              title: 'موقع المطعم',
                              line1: state.restaurantCart?.merchant?.name ?? 'المطعم',
                              line2: '${args.cartId ?? ''}',
                            ),
                          const SizedBox(height: 24),
                          Container(
                      ...
                      child: ElevatedButton(
                        onPressed: isPlacingOrder
                            ...
                            : () {
                                ...
                                context.read<OrdersBloc>().add(PlaceRestaurantOrderEvent());
                              },
