...
class _MultiDayCleaningOrderDetailsScreenState
  ...
  Future<void> _refresh({bool showLoading = false}) async {
    ...
    try {
      ...
      setState(() {
        ...
        _loading = false;
        _error = null;
        if (schedule != null && schedule.sessionById(_selectedSessionId) == null) {
  ...
  String _friendlyError(Object error) {
    ...
    if (text.contains('code')) return 'رمز التحقق غير صحيح أو انتهت صلاحيته.';
  ...
  Future<void> _confirmCompletion() async {
    ...
    final approved = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: const Text(
          'هل تم تنفيذ أعمال هذا اليوم بالشكل المطلوب؟',
        ),
        actions: [
          TextButton(
  ...
  String _statusLabel(CleaningBookingSessionModel session) {
    final api = session.statusLabel?.trim();
    ...
    for (final session in schedule.sessions) {
      for (final assignment in session.workerAssignments) {
        ...
        if (worker is Map) {
          ...
          if (nestedParsed != null && nestedParsed > 0) return nestedParsed;
        }
      }
    }
    return null;
  ...
  bool get _parentCompleted {
    if ((_envelope?.status ?? '').toLowerCase() == 'completed') return true;
    final schedule = _schedule;
    if (schedule == null || schedule.sessions.isEmpty) return false;
    final active = schedule.sessions.where((item) => !item.isCancelled).toList();
    return active.isNotEmpty && active.every((item) => item.isCompleted);
  }

  Widget _summaryCard(CleaningBookingScheduleModel schedule) {
    final progress = schedule.daysCount <= 0
        ? 0.0
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          AppText.bodySmall('${_timeLabel(session.time)} · ${_hours(session.hours)} ساعة'),
          if (price != null) ...[
