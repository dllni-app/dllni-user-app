...
class _RecurringCleaningScheduleRevisionScreenState
  ...
  void _removeVisit(int index) {
    if (_visits.length <= 1) {
      _message('يجب أن تبقى زيارة مستقبلية واحدة على الأقل في هذا التعديل.');
      return;
    }
    ...
    return await showDialog<bool>(
          ...
          builder: (dialogContext) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                Text(
                  'الزيارات المستقبلية الجديدة: ${preview.proposedSessionsCount}',
                ),
                if (preview.preservedSessionsCount > 0)
                  Text(
                    'الزيارات المحفوظة من السجل: ${preview.preservedSessionsCount}',
                  ),
