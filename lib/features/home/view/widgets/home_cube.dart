...
class _HomeCubeState extends State<HomeCube> {
  static const List<CubeFace> _faceOrder = [CubeFace.front, CubeFace.back, CubeFace.right, CubeFace.left];
          ...
          ? SizedBox.expand(
              child: Image.network(
                ...
                errorBuilder: (context, error, stackTrace) {
                  return ColoredBox(
                    color: base,
                    child: Center(child: Icon(Icons.local_offer_outlined, color: Colors.white.withValues(alpha: 0.9), size: 48)),
                  );
                },
                loadingBuilder: (context, child, loadingProgress) {
                  ...
                  return ColoredBox(
                    ...
                    child: Center(child: CircularProgressIndicator(color: base)),
                  );
                },
              ),
            )
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: FlutterCubeTransition(
        ...
        faces: {for (var i = 0; i < _faceOrder.length; i++) _faceOrder[i]: _faceForOffer(padded[i])},
      ),
    );
  }
}
