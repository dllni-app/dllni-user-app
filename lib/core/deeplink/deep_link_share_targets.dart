...
String deepLinkUserApiRoot() => '${deepLinkBase()}/api/v1/user';

/// Restaurant (RS) store detail — matches `GET /api/v1/user/restaurants/{id}`.
String restaurantUrl(int id) => '${deepLinkUserApiRoot()}/restaurants/$id';

/// Supermarket product — matches `GET /api/v1/user/supermarket/products/{id}`.
String supermarketProductUrl(int id) => '${deepLinkUserApiRoot()}/supermarket/products/$id';

/// Restaurant (RS) product — matches `GET /api/v1/user/products/{id}`.
String restaurantProductUrl(int id) => '${deepLinkUserApiRoot()}/products/$id';

/// Supermarket store — matches `GET /api/v1/user/supermarket/stores/{id}`.
String supermarketStoreUrl(int id) => '${deepLinkUserApiRoot()}/supermarket/stores/$id';

/// Vote follow-up — matches `.../restaurants/votes/{voteId}`.
String voteUrl(int id) => '${deepLinkUserApiRoot()}/restaurants/votes/$id';
...
String groupOrderUrl({int? id, String? shareToken}) {
  ...
  if (t != null && t.isNotEmpty) {
    return '${deepLinkUserApiRoot()}/restaurants/group-orders/${Uri.encodeComponent(t)}';
  }
  if (id != null && id > 0) {
    return '${deepLinkUserApiRoot()}/restaurants/group-orders/$id';
  }
  return deepLinkBase();
}
