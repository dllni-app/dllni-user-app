...
@lazySingleton
class LoggerInterceptor extends Interceptor {
  final _logger = Logger(printer: PrettyPrinter(methodCount: 0, lineLength: 100));

  @override
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    // طباعة سطر واحد فقط للطلب
  ...
  void onResponse(Response response, ResponseInterceptorHandler handler) {
    // طباعة الحالة والمسار فقط بشكل مختصر
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) {
    // طباعة الخطأ بشكل واضح ومفصل فقط عند حدوث خطأ
    final buffer = StringBuffer();
    buffer.writeln('❌ ERROR: ${err.response?.statusCode ?? 'UNKNOWN'} | ${err.requestOptions.path}');
